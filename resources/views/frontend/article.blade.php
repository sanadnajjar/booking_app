...
@section('content')
    ...
    @endforeach
        ...
        @auth
            ...
            @if ($article->isLiked())
                <a href="{{ route('unlike', ['id'=>$article->id, 'type'=>'Article']) }}" class="btn btn-primary btn-xs top-buffer">Unlike this article</a>
            @else
                <a href="{{ route('like', ['id'=>$article->id, 'type'=>'Article']) }}" class="btn btn-primary btn-xs top-buffer">Like this article</a>
            @endif

        @else

            <a href="{{ route('login') }}" class="btn btn-primary btn-xs top-buffer">Login to like this article</a>
...
@else
    <p><a href="{{route('login')}}" class="btn btn-primary btn-xs"></a>Login to add a comment</p>
@endauth
    ...
    <div class="collapse" id="collapseExample2">
        <div class="well">
            ...
            <form method="POST" action="{{route('addComment', ['article'=>$article->id, 'type'=>'Article'])}}" class="form-horizontal">
                <fieldset>
