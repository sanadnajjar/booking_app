                    ...
                    <div class="col-md-3 col-sm-6">
                        ...
                        <div class="thumbnail">
                            ...
                            <div class="caption">
                                ...
                                <p>{{\Illuminate\Support\Str::limit($room->description, 60)}}</p>
...
@else
    <p><a href="{{ route('login') }}" class="btn btn-primary btn-xs">Login to add a comment</a></p>
@endauth

    <div class="collapse" id="collapseExample">
        <div class="well">

            <form method="POST" action="{{route('addComment', ['object_id' => $object->id, 'type'=>'TouristObject'])}}" class="form-horizontal">
                <fieldset>
    ...
    <section>
        ...
        @foreach($object->articles as $article)
            ...
            <div class="articles-list">
                ...
                <p><b> {{$article->user->name}}</b>
                    <b>{{$article->user->surname}}</b>
                    <i>{{$article->created_at}}</i>
                </p>
                <p>{{Illuminate\Support\Str::limit($article->content, 250)}} </p> <a href="{{ route('article', ['id'=>$article->id]) }}">More</a>
            </div>
        ...
        @auth
            ...
            @if ($object->isLiked())
                <a href="{{ route('unlike', ['id'=>$object->id, 'type'=>'TouristObject']) }}" class="btn btn-primary btn-xs top-buffer">Unlike this object</a>
            @else
                <a href="{{ route('like', ['id'=>$object->id, 'type'=>'TouristObject']) }}" class="btn btn-primary btn-xs top-buffer">Like this object</a>
            @endif

        @else

            <a href="{{ route('login') }}" class="btn btn-primary btn-xs top-buffer">Login to like this object</a>
