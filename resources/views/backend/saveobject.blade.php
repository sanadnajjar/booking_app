...
<form {{$novalidate}} action="{{route('saveObject', ['id' => $object->id ?? null])}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
    <fieldset>
                    ...
                    @foreach($chunked_photos as $photo)
                        ...
                        <div class="col-md-3 col-sm-6">
                            <div class="thumbnail">
                                ...
                                <div class="caption">
                                    <p><a href="{{route('deletePhoto', ['id' => $photo->id])}}" class="btn btn-danger btn-xs" role="button">Delete</a></p>
                                </div>
            ...
            <ul class="list-group">
                @foreach($object->articles as $article)
                    <li class="list-group-item">
                        {{$article->title}} <a href="{{route('deleteArticle', ['id' => $article->id])}}" class="btn btn-danger btn-xs">delete</a>
                    </li>
                @endforeach
...
<div class="col-lg-10 col-lg-offset-2">
    ...
    <form {{$novalidate}} action="{{route('saveArticle', ['id'=> $object->id ?? null])}}" method="POST" class="form-horizontal">
        <fieldset>

            <div class="form-group">
                <label for="textTitle" class="col-lg-2 control-label">Title *</label>
                <div class="col-lg-10">
                    <input name="title" required type="text" class="form-control" id="textTitle" placeholder="" value="{{old('title')}}">
                </div>
            </div>
